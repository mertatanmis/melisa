        ...
        <div className="grid lg:grid-cols-2 gap-12 items-start">
              ...
              <div className="grid grid-cols-2 gap-6">
                {features.map((feature, index) => (
                  <div key={index} className="flex items-start space-x-4">
                    ...
                    <div>
                      <h4 className="font-semibold text-corporate-700 mb-1">
                        {feature.title}
                      </h4>
          ...
          <div className="relative h-[500px] overflow-hidden rounded-2xl shadow-lg">
            <button 
              ...
              className="absolute left-4 top-1/2 z-10 -translate-y-1/2 bg-white/80 hover:bg-white p-3 rounded-full shadow-md hover:shadow-lg transition-all duration-300"
            >
              <ChevronLeft className="w-6 h-6 text-corporate-600" />
            </button>
            <button 
              onClick={goToNext} 
              className="absolute right-4 top-1/2 z-10 -translate-y-1/2 bg-white/80 hover:bg-white p-3 rounded-full shadow-md hover:shadow-lg transition-all duration-300"
            >
              <ChevronRight className="w-6 h-6 text-corporate-600" />
            </button>
              ...
              <div className="flex h-full">
                {locationImages.map((image, index) => (
                  <div key={index} className="min-w-full h-full relative">
                    ...
                    />
                    <div className="absolute bottom-0 inset-x-0 bg-white/95 backdrop-blur-sm p-6">
                      <h4 className="text-lg font-semibold text-corporate-700 mb-1">
                        {image.caption}
                      </h4>
                      <p className="text-sm text-neutral-600">
                        {image.description}
            ...
            {/* Dots Navigation */}
            <div className="absolute bottom-28 inset-x-0 flex justify-center space-x-2">
